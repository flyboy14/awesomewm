...
awesompd.STOPPED = "<span font='Visitor TT2 BRK 12' color='#D83B59'>Stopped</span>"
awesompd.DISCONNECTED = "<span font='Visitor TT2 BRK 12' color='#D83B59'>Disconnected</span>"
...
-- Default user options
   ...
   instance.ldecorator = "q "
   instance.rdecorator = " p"
...
function awesompd:command_show_menu()
   ...
   function()
      ...
      if self.recreate_menu then 
         ...
         self.main_menu = awful.menu({ items = new_menu, theme = { width = 200, bg_normal="#4B3B51", bg_focus="#4B3B51", fg_normal="#DBCFE0", fg_focus="#D83B59" } }) 
...
function awesompd:menu_jamendo_top()
   ...
   function ()
      ...
      if not track_table then
         self:show_notification("Can't connect to Jamendo server", "Please check your network connection",nil,2)
      else
   ...
   if not track_line or string.len(track_line) == 0 then
	 ...
	 if self.status ~= awesompd.STOPPED then
	    ...
	    self.recreate_list = true
         ...
         local display_name, force_update = title .. "<span color='#cecece'> - " .. artist .. "</span>", false
