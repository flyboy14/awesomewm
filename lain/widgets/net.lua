...
local notify_fg    = require("beautiful").fg_normal
...
function net.get_device()
    ...
    ws = f:read("*a")
    f:close()
    ws = ws:match("wl%w+: UP") or ws:match("enp%w+: UP") or ws:match("enp%w+: UNKNOWN")
    if ws ~= nil then
        return ws:match("(%w+):")
    else
        return "network off"
...
local function worker(args)
    ...
    function update()
        ...
        net_now.sent = string.gsub(string.format('%.0f', net_now.sent), ",", ".")

        net_now.received = (now_r - net.last_r) / timeout / units
        net_now.received = string.gsub(string.format('%.0f', net_now.received), ",", ".")
