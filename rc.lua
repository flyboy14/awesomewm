...
-- plugged
  if (batstate() == 'Cable plugged') then
    ...
    show_smth(nil, "Кабель подключён", beautiful.widget_ac, 2, nil, nil, nil, nil)
    state_changed = 0
  end
    return '<span background="#121212" font="Fixed 14"><span rise="200" font="Visitor TT2 BRK 13"color="#46A8C3" rise="1600">AC</span></span>'
    -- critical
 ...
 elseif (args[2] >= 90) then
   ...
   if (batstate() == 'Discharging') then
       if (state_changed == 1) then
    show_smth(nil, "Кабель отключён", beautiful.widget_ac, 2, nil, nil, nil, nil)
    state_changed = 0
    end 
    return '<span background="#121212" color="#e54c62" font="Fixed 14"><span font="Fixed 9">↓ <span font="Visitor TT2 BRK 12" rise="200">' .. args[2] .. '% </span></span></span>'
   elseif (batstate() == 'Charging' and args[2] ~= 100) then
       if (state_changed == 1) then
    show_smth(nil, "Кабель подключён", beautiful.widget_ac, 2, nil, nil, nil, nil)
    state_changed = 0 
  end
    return '<span background="#121212" font="Fixed 14"><span font="Fixed 9" color="#7AC82E">↑ <span font="Visitor TT2 BRK 12" rise="200">' .. args[2] .. '% </span></span></span>'
   else 
    return '<span background="#121212" font="Fixed 14" color="#46A8C3"><span font="Fixed 9">⚡ <span font="Visitor TT2 BRK 12" rise="200">' .. args[2] .. '% </span></span></span>' end
end, 1, 'BAT0')
...
awful.rules.rules = {
    -- All clients will match this rule.
      ...
      properties = { floating = true } },
            ...
            "Samowar", "Virt-manager", "Eiskaltdcpp", "Deadbeef", "VirtualBox" } },
